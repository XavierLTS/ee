...
const state = {
    ...
    test: {
        stage: 'idle', 
        testerName: '',
        history: [], // Stores { id, movieId, layoutKey, time, status }
        maxTests: 6,
        targetMovie: null,
        targetLayout: null,
        startTime: 0,
...
function setState(key, value) {
    ...
    renderApp();
    renderTaskBar(); // Always update taskbar as it contains translated text
}
...
function startRandomTask() {
    ...
    state.test.timerInterval = setInterval(() => {
        ...
        // 20秒超时检查
        if (state.test.elapsedTime >= 20) {
            state.test.elapsedTime = 20;
            const timerDisplay = document.getElementById('task-timer-display');
            if (timerDisplay) {
                 timerDisplay.innerText = '20.0s';
                 timerDisplay.style.color = '#ef4444'; // 超时变红
            }
            endTask(false, true); // 触发失败（超时）
        } else {
            const timerDisplay = document.getElementById('task-timer-display');
            if (timerDisplay) {
                 timerDisplay.innerText = state.test.elapsedTime.toFixed(1) + 's';
                 // 临近超时预警 (15秒后)
                 if(state.test.elapsedTime > 15) timerDisplay.style.color = '#f97316';
                 else timerDisplay.style.color = 'var(--test-accent)';
            }
        }
    }, 100);
}

function endTask(success, isTimeout = false) {
    clearInterval(state.test.timerInterval);
    const t = TRANSLATIONS[state.lang];
    
    // 成功找到 或 超时失败 都记录
    if (success || isTimeout) {
        // 重要修改：存储 Keys 而不是翻译后的文本
        const record = {
            id: state.test.history.length + 1,
            movieId: state.test.targetMovie.id, // 存 ID
            layoutKey: state.test.targetLayout, // 存 Key ('list', 'grid', 'search')
            time: success ? state.test.elapsedTime.toFixed(2) : 'TIMEOUT',
...
function showFailModal() {
    const wrapper = document.createElement('div');
    wrapper.id = 'success-modal-wrapper'; // 重用ID以便清理
    wrapper.style.cssText = 'position: fixed; inset: 0; z-index: 2000; display: flex; align-items: center; justify-content: center;';
    const t = TRANSLATIONS[state.lang];
    
    wrapper.innerHTML = `
        <div class="modal-overlay" style="position: absolute; inset: 0; background: rgba(0,0,0,0.7); backdrop-filter: blur(4px);"></div>
        <div class="success-modal" style="position: relative; background: var(--bg-card); padding: 2rem; border-radius: 1rem; box-shadow: 0 20px 50px rgba(0,0,0,0.3); text-align: center; min-width: 300px; max-width: 90%; margin: 1rem; border: 2px solid #ef4444; animation: popIn 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275);">
            ...
            <div style="display:flex; gap:1rem; justify-content:center">
                <button onclick="document.getElementById('success-modal-wrapper').remove(); startRandomTask()" class="btn btn-primary" style="padding:0.75rem 1.5rem; background-color:#ef4444">
                    ${t.startNext}
                </button>
            </div>
        </div>
...
function renderTaskBar() {
    ...
    if (state.test.stage === 'idle') {
        container.innerHTML = `
            <div class="task-bar" style="border: 1px solid rgba(255,255,255,0.1); backdrop-filter: blur(10px); background: var(--test-bg); padding: 1.25rem 2rem; border-radius: 99px; box-shadow: 0 10px 40px rgba(0,0,0,0.3); margin-bottom: 2rem;">
                <div style="font-weight:bold; font-size:1.1rem; display:flex; align-items:center; gap:0.75rem; color: var(--test-text);">
                    <span style="background:var(--test-accent); color:var(--test-bg); padding:3px 10px; border-radius:99px; font-size:0.75rem; font-weight:800; letter-spacing: 0.05em;">TEST</span>
                    ${t.testMode}
                </div>
                <button onclick="initTestSession()" class="task-btn" style="background: white; color: var(--test-bg); border-radius: 99px; padding: 0.6rem 1.5rem; box-shadow: 0 4px 12px rgba(255,255,255,0.2);">
                    ${t.startTest}
                </button>
            </div>
        `;
    ...
    if (state.test.stage === 'testing' && state.test.targetMovie) {
        ...
        const layoutName = t.layouts[state.test.targetLayout]; // Fix: Use translated layout name directly
        const progress = `${state.test.history.length + 1} / ${state.test.maxTests}`;

        container.innerHTML = `
            <div class="task-bar" style="background:rgba(20, 20, 30, 0.85); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.1); border-radius: 24px; padding: 1rem 1.5rem; color:white; box-shadow: 0 15px 50px rgba(0,0,0,0.3); margin-bottom: 1.5rem;">
                <div class="task-info-group" style="flex: 1; align-items: stretch;">
                    <div style="display:flex; flex-direction:column; min-width: 60px; justify-content: center;">
                        <span class="task-label" style="font-size:0.65rem; color: rgba(255,255,255,0.5);">${t.taskProgress}</span>
                        <div class="task-value" style="color:white; font-size:1.1rem;">${progress}</div>
                    </div>
                    <div style="width: 1px; background: rgba(255,255,255,0.1); margin: 0 1rem;"></div>
                    <div style="flex: 1; display:flex; flex-direction:column; justify-content: center;">
                        <span class="task-label" style="font-size:0.65rem; color: rgba(255,255,255,0.5);">${t.colTarget}</span>
                        <div class="task-value" style="color:var(--test-accent); font-size:1.2rem; font-weight:800; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 180px; text-shadow: 0 0 20px rgba(74, 222, 128, 0.3);">${movieTitle}</div>
                    </div>
                    <div style="width: 1px; background: rgba(255,255,255,0.1); margin: 0 1rem; display:none; @media(min-width:768px){display:block}"></div>
                    <div style="display:none; @media(min-width:768px){display:flex; flex-direction:column; justify-content: center;}">
                        <span class="task-label" style="font-size:0.65rem; color: rgba(255,255,255,0.5);">${t.colLayout}</span>
                        <div class="task-value" style="color:white; font-size:1rem;">${layoutName}</div>
                    </div>
                </div>
                
                <div style="display:flex; align-items:center; gap:1rem; padding-left: 1rem; border-left: 1px solid rgba(255,255,255,0.1);">
                    <div class="task-timer-group">
                        <span class="task-limit-label">${t.timeLimitHint}</span>
                        <div id="task-timer-display" class="task-timer" style="font-size:1.4rem; background:transparent; padding:0; color:white; font-variant-numeric: tabular-nums;">0.0s</div>
                    </div>
                    <button onclick="endTask(false)" class="task-btn secondary" style="background:rgba(255,255,255,0.1); color:white; padding: 0.6rem; border-radius: 50%; aspect-ratio: 1; display:flex; align-items:center; justify-content:center;">
    ...
    if (state.test.stage === 'testing' && !state.test.targetMovie) {
         container.innerHTML = `
            <div class="task-bar" style="background:rgba(20, 20, 30, 0.9); backdrop-filter: blur(20px); border: 1px solid rgba(255,255,255,0.1); border-radius: 24px; padding: 1.25rem 2rem;">
                <div style="font-weight:bold; font-size:1.1rem; color:rgba(255,255,255,0.6)">${t.taskAborted}</div>
                <button onclick="startRandomTask()" class="task-btn" style="background:white; color:black; border-radius:99px; padding: 0.6rem 1.5rem;">
                    ${t.startTaskBtn}
                </button>
            </div>
        `;
...
function generateSummaryLayout() {
    ...
    // 只计算成功的耗时
    const successfulTasks = history.filter(h => h.status === 'success');
    const totalTime = successfulTasks.reduce((acc, curr) => acc + parseFloat(curr.time), 0).toFixed(2);
    const avgTime = successfulTasks.length > 0 ? (totalTime / successfulTasks.length).toFixed(2) : '0.00';
    
    // 动态生成行，支持语言切换
    const rows = history.map(row => {
        // 获取电影信息
        const movie = ALL_MOVIES.find(m => m.id === row.movieId);
        const movieTitle = movie ? (state.lang === 'zh' ? movie.title : movie.titleEn) : 'Unknown';
        
        // 获取布局名称
        const layoutName = t.layouts[row.layoutKey];
        
        // 处理时间显示（如果是超时，显示红色失败）
        let timeDisplay = row.time;
        if (row.status === 'timeout') {
            timeDisplay = `<span style="color:#ef4444">${t.failedReason}</span>`;
        }
        ...
        <tr>
            ...
            <td>${movieTitle}</td>
            <td>${layoutName}</td>
            <td style="font-family:monospace; font-weight:bold">${timeDisplay}</td>
        </tr>
    `}).join('');

    return `
        <div class="scroll-area summary-container">
            <div class="summary-header">
                <div style="font-size:3rem; margin-bottom:0.5rem">📊</div>
                <h2 style="font-size:2rem; font-weight:800; margin-bottom:0.5rem; color: var(--text-main);">${t.summaryTitle}</h2>
                <div style="color:var(--text-sub)">${t.summaryTester}: <strong>${state.test.testerName}</strong></div>
            </div>

            <div class="summary-stats-grid">
                <div class="summary-stat-card">
                    <div style="font-size:0.875rem; color:var(--text-sub); margin-bottom:0.5rem; text-transform: uppercase; letter-spacing: 0.05em;">${t.summaryTotal}</div>
                    <div style="font-size:2.5rem; font-weight:800; color:var(--primary); line-height: 1;">${totalTime}<span style="font-size:1rem; margin-left:2px;">s</span></div>
                </div>
                <div class="summary-stat-card">
                    <div style="font-size:0.875rem; color:var(--text-sub); margin-bottom:0.5rem; text-transform: uppercase; letter-spacing: 0.05em;">${t.summaryAvg}</div>
                    <div style="font-size:2.5rem; font-weight:800; color:var(--accent-orange); line-height: 1;">${avgTime}<span style="font-size:1rem; margin-left:2px;">s</span></div>
                </div>
            </div>
            ...
            <div style="display:flex; flex-wrap:wrap; justify-content:center; gap:1rem; margin-top:2rem; padding-bottom: 2rem;">
                <button onclick="exportData()" class="btn btn-primary" style="padding:0.8rem 2rem; box-shadow: var(--shadow-md);">
                    <i data-lucide="download"></i> ${t.exportData}
                </button>
                <button onclick="restartTest()" class="btn" style="border:1px solid var(--border); padding:0.8rem 2rem;">
...
function exportData() {
    const data = {
        ...
        results: state.test.history.map(h => ({
            ...h,
            // 导出时添加人类可读信息，默认用英文以通用
            movieEn: ALL_MOVIES.find(m => m.id === h.movieId)?.titleEn,
            layout: h.layoutKey
        }))
    };
...
function generateDetailLayout() {
    ...
    return `
        <div class="detail-view">
            ...
            <div class="detail-hero-section" style="position:relative; padding-bottom:4rem;">
                <div class="detail-hero-bg ${movie.posterClass}"></div>
                <div style="position:absolute; inset:0; background:linear-gradient(to top, var(--bg-card) 5%, transparent 80%); z-index:1;"></div>
                <div class="detail-content-wrapper" style="z-index:2;">
                    <div class="detail-poster-large" style="border:1px solid rgba(255,255,255,0.3); box-shadow: 0 20px 40px rgba(0,0,0,0.4);">
                        <i data-lucide="film" size="64" class="text-white opacity-50"></i>
                    </div>
                    <h1 style="font-size: 2rem; font-weight: 800; text-shadow: 0 4px 12px rgba(0,0,0,0.5); margin: 0; line-height:1.2; text-align:center;">${info.title}</h1>
                    
                    <div class="detail-meta-tags">
                        <span class="detail-meta-tag" style="background:rgba(255,255,255,0.2); backdrop-filter:blur(10px); border:1px solid rgba(255,255,255,0.1);">${movie.year}</span>
                        <span class="detail-meta-tag" style="background:rgba(255,255,255,0.2); backdrop-filter:blur(10px); border:1px solid rgba(255,255,255,0.1);">${info.genre}</span>
                        <span class="detail-meta-tag" style="background:var(--accent-orange); color:white; border:none; box-shadow: 0 4px 10px rgba(249, 115, 22, 0.4);">
                            ★ ${movie.rating}
                        </span>
                    </div>

                    <div class="detail-info-grid" style="background: rgba(var(--bg-card-rgb), 0.5); border:1px solid rgba(255,255,255,0.1); backdrop-filter:blur(12px);">
                        <div class="info-item">
                            <span class="info-label" style="opacity:0.8;">${t.director}</span>
                            <span class="info-value">${info.director}</span>
                        </div>
                        <div class="info-item">
                            <span class="info-label" style="opacity:0.8;">${t.starring}</span>
                            <span class="info-value">${info.cast}</span>
                        </div>
                    </div>

                    <div class="detail-actions">
                        <button class="btn btn-primary" style="padding:0.8rem 2.5rem; border-radius:99px; font-size:1rem; box-shadow: 0 4px 15px rgba(37, 99, 235, 0.3);">
                            <i data-lucide="play" fill="currentColor"></i> ${t.playNow}
                        </button>
                        <button class="btn" style="background:rgba(255,255,255,0.15); color:white; border-radius:99px; padding:0.8rem; backdrop-filter:blur(4px); border:1px solid rgba(255,255,255,0.2);">
                            <i data-lucide="heart"></i>
                        </button>
                    </div>
                </div>
            </div>
            <div class="detail-synopsis" style="flex: 1; padding-bottom: 100px; margin-top: -1rem; padding-top: 1rem; position:relative; z-index:2;">
                <h3 style="font-size:1.25rem; font-weight:bold; margin-bottom:1rem; padding-bottom:0.5rem; display:flex; align-items:center; gap:0.5rem;">
                    <span style="width:4px; height:1.2em; background:var(--primary); border-radius:2px;"></span>
                    ${t.synopsis}
                </h3>
                <p style="line-height:1.8; color:var(--text-sub); font-size:1.05rem; text-align:justify;">
                    ${info.synopsis}
                </p>
            </div>
        </div>
...
function generateListLayout() {
    ...
    if (state.listExpandedGenre) {
        ...
        yearsHTML = years.map(year => `
            <button onclick="setState('listSelectedYear', '${year}')" class="btn ${state.listSelectedYear == year ? 'active' : ''}" style="font-size:0.8rem; padding:0.25rem 0.6rem; border:1px solid var(--border); border-radius:99px;">
                ${year}
            </button>
        `).join('');
    }
            ...
            <li style="margin-bottom: 0.25rem;">
                <button onclick="setState('listExpandedGenre', '${isExpanded ? '' : genre}'); setState('listSelectedYear', null)" class="nav-item-btn ${isExpanded ? 'active' : ''}">
                    <div class="flex-center gap-sm">
                        <span style="width:6px; height:6px; border-radius:50%; background-color:${isExpanded ? 'var(--primary)' : 'var(--text-muted)'}; transition: background-color 0.3s;"></span>
                        ${genre}
                    </div>
                    <i data-lucide="${isExpanded ? 'chevron-down' : 'chevron-right'}" size="14" style="opacity:0.5; transition: transform 0.2s;"></i>
                </button>
                ${isExpanded ? `<div class="sub-nav" style="display:flex; flex-wrap:wrap; gap:0.5rem; padding:0.5rem 0.5rem 0.5rem 1.5rem;">${yearsHTML}</div>` : ''}
            </li>
        `;
    }).join('');
    ...
    const itemsHTML = filteredMovies.length > 0 ? filteredMovies.map(m => {
        ...
        <div onclick="viewMovie(${m.id})" class="list-item">
            <div class="poster-thumb ${m.posterClass}" style="box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
                <i data-lucide="film" size="20" style="opacity:0.7;"></i>
            </div>
            <div style="flex:1; min-width:0">
                <h3 style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; font-weight:700;">${info.title}</h3>
                <div class="item-meta">
                    <span style="background:var(--bg-body); padding:2px 6px; border-radius:4px; font-size:0.75rem;">${m.year}</span> 
                    <span style="color:var(--text-muted);">•</span> 
                    <span>${info.director}</span>
                </div>
            </div>
            <div class="rating-badge" style="flex-shrink:0; background:transparent; color:var(--accent-orange); font-size:1rem;">
                <i data-lucide="star" size="14" fill="currentColor"></i> ${m.rating}
            </div>
        </div>`;
    ...
    return `
        <div class="layout-split">
            <aside class="sidebar custom-scrollbar">
                <div class="sidebar-header" style="background:var(--bg-sidebar); backdrop-filter:blur(10px);">${t.navTitle}</div>
                <ul class="nav-list">${sidebarHTML}</ul>
            </aside>
            <div class="content-area scroll-area">
                <div class="section-title" style="position:sticky; top:0; background:var(--bg-card); z-index:10; padding-bottom:1rem; border-bottom:1px solid transparent;">
                    ${state.listExpandedGenre || t.allMovies}
                    ${state.listSelectedYear ? `<span class="tag-badge" style="background:var(--primary); color:white;">${state.listSelectedYear}</span>` : ''}
                </div>
                <div style="padding-top:0.5rem;">${itemsHTML}</div>
            </div>
        </div>
    `;
}
    ...
    const cardsHTML = filteredMovies.map(m => {
        ...
        <div onclick="viewMovie(${m.id})" class="grid-card">
            <div class="card-poster ${m.posterClass}">
                ...
                <div style="position:absolute; top:8px; right:8px; background:rgba(0,0,0,0.6); color:white; padding:2px 6px; border-radius:4px; font-size:10px; font-weight:bold; display:flex; gap:2px; align-items:center; backdrop-filter:blur(4px);">
                    <i data-lucide="star" size="10" fill="#facc15" color="#facc15"></i> ${m.rating}
                </div>
            </div>
            <div class="card-body">
                <div class="card-title">${info.title}</div>
                <div style="font-size:0.75rem; color:var(--text-sub); margin-bottom:0.5rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${info.director}</div>
                <div class="card-footer" style="border-top:1px solid var(--border); padding-top:0.5rem; margin-top:auto;">
                    <span class="tag-badge" style="font-size:10px; padding:2px 6px;">${m.year}</span>
                    <span style="font-size:10px; opacity:0.8;">${info.genre}</span>
                </div>
            </div>
        </div>`;
    }).join('');
...
function generateMovieItemsHTML(movies) {
     ...
     return movies.map(m => {
        const info = getMovieInfo(m);
        return `
        <div onclick="viewMovie(${m.id})" class="list-item" style="border:none; background:transparent; margin-bottom:0; padding:1rem 0; border-bottom:1px solid var(--border);">
            <div class="poster-thumb ${m.posterClass}" style="width:60px; height:80px; border-radius:6px;"></div>
            <div style="flex:1; min-width:0">
                <div class="flex-between">
                    <h3 style="font-size:1.125rem; white-space:nowrap; overflow:hidden; text-overflow:ellipsis">${info.title}</h3>
                    <div class="rating-badge" style="background:var(--bg-body);"><i data-lucide="star" size="10" fill="currentColor"></i> ${m.rating}</div>
                </div>
                <div class="item-meta" style="margin-top:0.25rem">
                    <span class="tag-badge">${m.year}</span>
                    <span style="margin-left:0.5rem; color:var(--text-main); font-size:0.8rem;">${info.genre}</span>
                    <span style="color:var(--text-muted); margin:0 0.25rem;">·</span>
                    <span style="color:var(--text-sub); font-size:0.8rem;">${info.director}</span>
                </div>
...
function generateSearchLayout() {
    ...
    return `
        <div style="height: 100%; display:flex; flex-direction:column">
            <div class="search-hero" style="padding: 2.5rem 1.5rem; border-bottom: none; box-shadow: 0 4px 20px -10px rgba(0,0,0,0.1); z-index:10;">
                <div class="search-title" style="font-size: 1.75rem; margin-bottom: 1.25rem;">${t.searchTitle}</div>
                <div class="search-input-wrapper">
                    <input oninput="handleSearchInput(this.value)" value="${state.searchQuery}" type="text" placeholder="${t.searchPlaceholder}" class="search-input" style="padding: 1.2rem 1.2rem 1.2rem 3.5rem; font-size: 1rem; border: none; box-shadow: 0 4px 15px rgba(0,0,0,0.08);">
                    <i data-lucide="search" class="search-icon" style="left: 1.5rem;"></i>
                </div>
                <div class="tags-container" style="margin-top: 1rem;">
                    ${t.tags.map(tag => `
                        <button onclick="setState('searchQuery', '${tag.value}')" class="search-tag ${state.searchQuery === tag.value ? 'active' : ''}" style="font-size: 0.8rem; padding: 0.4rem 1rem;">
                            ${tag.label}
                        </button>
                    `).join('')}
                </div>
            ...
            <div class="scroll-area">
                <div class="container" style="max-width:800px; padding: 1.5rem">
                    <div class="flex-between" style="border-bottom:1px solid var(--border); padding-bottom:0.5rem; margin-bottom:1rem; font-size:0.75rem; font-weight:bold; color:var(--text-muted); text-transform:uppercase">
                        ...
                        <button id="search-clear-btn" onclick="setState('searchQuery', '')" style="color:var(--primary); display:${state.searchQuery ? 'block' : 'none'}">${t.clear}</button>
                    </div>
